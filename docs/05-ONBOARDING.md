...
> **2026-08-18 简化。** 21 屏砍到 **6 步问题 + 一个梦想提问**，
> 并且把结尾的「今天想让 Wren 带什么」换成 **「你梦想的生活是什么样子」**。
>
> ```
> Welcome → 名字 → 想改变什么(语音) →〔回应〕→ 重要的人 → 工作 →〔换挡〕
>        → 梦想的生活(语音) → 转写确认 → 生成 → 三选一 → 播放 → 承诺 → Home
> ```
>
> **为什么换：** 「今天带什么」把第一次体验锁在了一件小事上，而新用户还没有
> 「今天」的语境 —— 她刚下载，脑子里装的是那个长期的东西。梦想提问一上来就
> 拿到最大的那块料，后面每天的短会话都能从它派生。
>
> 换挡屏的文案也跟着变了：`Don't be careful with it.` —— 这是我们对 Stella
> 那句 "Don't be realistic" 的回应。不是叫她编大的，是叫她别把真的说小了。
>
> **生成侧新增 `dream` 模式**（`generate.js`）：Land 段念她的原话，See 段
> 把她放进那个场景里，carry 句变成「我可以想要整栋房子 / 整个数目」。
> 一条硬规则：**梦境里只放亲密关系** —— 把经理写进她的梦想厨房，整段就废了。
>
> 下面是简化前的完整推演，保留作为依据。
